...
scutum.bundles = {
	...
	'billboard': ['node_modules/billboard.js/dist/billboard.min.js', 'assets/css/plugins/billboard.min.css'],
	...
	'optiscroll': ['node_modules/optiscroll/dist/jquery.optiscroll.min.js', 'assets/css/plugins/optiscroll.min.css'],
...
scutum.helpers.scrollbar = {
	...
	'disable': function (height) {
		...
		scutum.$body.css({'overflow': 'hidden'});
	...
	'custom': function (el) {
		if (scutum.config.customScrollbar) {
			...
			if ($el.length) {
				scutum.require(['optiscroll'], function () {
					$el.each(function () {
						var $this = $(this);
						if ($this.attr('data-sc-scrollbar') === 'visible-v') {
							$this.addClass('optiscroll-always-visible-v');
						}
						$this.addClass('optiscroll').optiscroll()
					})
				});
			}
		}
...
scutum.helpers.iCheck = {
	...
	/**
	 * Toggle <tr>,<li> element on checbox change
	 */
				...
				complete: function () {
					if (slideToogle !== 'slideUp') {
						if (scutum.$sidebarMain.hasClass('sc-js-scroll-to-active')) {
							var container = scutum.config.customScrollbar ? scutum.$sidebarMain.find(".optiscroll-content") : scutum.$sidebarMain.find(".uk-offcanvas-bar");
...
scutum.forms.common.select2 = function (parent) {
	var $el = parent ? $(parent).find('select') : $('[data-sc-select2]');
	if ($el.length) {
	...
	if ($el.length) {
		scutum.require(['raty'], function () {
			$el.each(function (index, item) {
				...
				$element.raty({
					starType: 'span',
					cancel: !!$element.attr('data-sc-raty-cancel'),
					half: !!$element.attr('data-sc-raty-half'),
					score: function () {
						return $element.attr('data-sc-raty-score');
					},
					scoreName: $element.attr('data-sc-raty-score-name') ? $element.attr('data-sc-raty-score-name') : 'score',
					number: $element.attr('data-sc-raty-number') ? $element.attr('data-sc-raty-number') : 5,
					readOnly: !!$element.attr('data-sc-raty-read-only')
				});
			});
		});
	}
